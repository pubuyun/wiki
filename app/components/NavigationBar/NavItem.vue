        ...
        >
            <NavigationMenuLink v-for="link in links" :key="link.to" as-child>
                <NuxtLink
                    :to="link.to"
                    @pointerdown.stop
                    class="block px-4 py-2 text-xl text-primary-deep no-underline -outline-offset-2 first:rounded-t-2xl last:rounded-b-2xl hover:bg-primary-norm focus-visible:bg-primary-norm focus-visible:outline-2 focus-visible:outline-primary-deep"
                >
                    {{ link.label }}
                </NuxtLink>
            </NavigationMenuLink>
