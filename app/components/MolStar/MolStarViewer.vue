...
const render = async () => {
    ...
    const pdbProvider =
        query["pdb-provider"]?.trim()?.toLowerCase() || props.pdbProvider;
    const emdbProvider =
        query["emdb-provider"]?.trim()?.toLowerCase() || props.emdbProvider;
    const mapProvider =
        query["map-provider"]?.trim()?.toLowerCase() || props.mapProvider;
    ...
    const transparency =
        query["transparency"]?.trim()?.toLowerCase() || props.transparency;
    const preferWebgl1 =
        query["prefer-webgl1"]?.trim() === "1" || props.preferWebgl1;
    const allowMajorPerformanceCaveat =
        query["allow-major-performance-caveat"]?.trim() === "1" ||
        props.allowMajorPerformanceCaveat;
    const powerPreference =
        query["power-preference"]?.trim()?.toLowerCase() ||
    ...
    Viewer.create("molStarWrapper", {
        ...
        pickPadding: isNaN(parseFloat(pickPadding))
            ? 1
            : parseFloat(pickPadding),
        ...
        const snapshotUrlType =
            query["snapshot-url-type"]?.toLowerCase()?.trim() ||
        ...
        const structureUrlIsBinary =
            query["structure-url-is-binary"]?.trim() === "1" ||
        ...
        if (Array.isArray(structureUrl)) {
            ...
            const structureUrlFormat =
                ...
                exchangeFileFormatToMolstarFormat(
                    structureUrl.split(".").pop(),
                );
            viewer.loadStructureFromUrl(
                structureUrl?.trim(),
                structureUrlFormat,
        ...
        const modelArchive =
            query["model-archive"]?.trim() || props.modelArchive;
